...
const size_t buzzer_partition_size = sizeof(buzzer_partition) / sizeof(buzzer_partition[0]);

void Buzzer::Power() {
	if (state == State::OFF) {
	        state = State::ON;
	        std::cout << "Buzzer allumé." << std::endl;

	        buzzer_play_note_by_name(&htim3, notes, notes_size, "G5");

	    } else {
	        state = State::OFF;
	        std::cout << "Buzzer éteint." << std::endl;
...
void Buzzer::SetState(State newState) {
    state = newState;
    std::cout << "État du buzzer défini sur : "
...
void buzzer_play_note_by_name(TIM_HandleTypeDef *_htim, TypeDef_Note *_notes, size_t _notes_sz, const char* _name) {
    for (int j = 0; j < sizeof(buzzer_partition) / sizeof(buzzer_partition[0]); j++) {
